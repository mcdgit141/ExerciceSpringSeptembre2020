...
class LibrairieApplicationTests {
	...
	@Test
	@DisplayName("Doit retourner une erreur 500 quand pas de librairie créée")
	void test_create_2(){
		// given
	...
	void test_create_3(){
		// given
		librairieRepository.enregistrerLibrairie(ASSOCIATIVE_LIBRAIRIE_MONTREUIL);
		//when
		ResponseEntity<Librairie> response = testRestTemplate.getForEntity("/v1/gestlibrairie/listeid/1", Librairie.class);
		//then
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
		assertThat(response.getBody().getTypeLibrairie()).isEqualTo(TypeLibrairie.associative);
	}
}
